...
// Default Pune coordinates for campus messes
...
export default function MessDirectionsModal({
  ...
  const [routeStats, setRouteStats] = useState({ distance: '350m', duration: '4 min walk' });
  ...
  // Generate Leaflet Map with OSRM Real Road Route
  const htmlContent = `
    <!DOCTYPE html>
    <html>
    <head>
  ...
  const handleOpenExternalMaps = () => {
    ...
    const url = `https://www.google.com/maps/dir/?api=1&destination=${messLat},${messLng}&destination_place_id=${encoded}`;
          ...
          <View style={[styles.detailsCard, { backgroundColor: colors.cardBg, borderColor: colors.cardBorder }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              ...
              <View style={styles.distPill}>
                ...
                <Text style={styles.distPillText}>{routeStats.distance} ({routeStats.duration})</Text>
              </View>
            </View>

            <TouchableOpacity style={styles.externalBtn} onPress={handleOpenExternalMaps} activeOpacity={0.85}>
              <ExternalLink size={16} color="#FFFFFF" />
              <Text style={styles.externalBtnText}>Open in Google Maps App</Text>
            </TouchableOpacity>
          </View>
        </BlurView>
      </View>
